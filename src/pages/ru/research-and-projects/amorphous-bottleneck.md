...
<div class="project-gallery">
  <figure class="project-figure project-figure-wide">
    <img src="/images/amorphous-bottleneck-overview.jpg" width="800" height="397" alt="Схема трёх составляющих аморфного узкого места" loading="lazy" />
    <figcaption>Аморфное узкое место объединяет ошибки распознавания, непоследовательность разметки и ограничения оценки.</figcaption>
  </figure>
...
### 2. Асимметричная оценка безопасности
...
graph TD
    ...
    C --> D[Greedy Forward Selection по оценке безопасности]
    D --> E[Этап III: Weighted Boxes Fusion]
    E --> F[Оптимизированный ансамбль из 6 моделей]
```

### Анализ обнаружений и пропусков
Вместо одной усреднённой метрики AP проводится проверка каждого объекта. Для каждого изображения и каждой эталонной рамки при $IoU \ge 0,5$ учитываются:
...
## Парадокс точности и «призрачные» обнаружения

При пороге 0,50 точность ансамбля снижается с 0,7125 до 0,5457. Чтобы понять причину, я вручную проверил **168 «призрачных» обнаружений**: уверенные прогнозы ансамбля с $IoU < 0,05$ относительно любой эталонной разметки.

Проверка 168 случаев на 114 изображениях показала **парадокс точности**:
