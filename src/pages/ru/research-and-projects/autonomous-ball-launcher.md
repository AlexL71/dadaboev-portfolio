...
description: "Автономная система слежения за целью с цветовой фильтрацией в OpenCV, позиционированием сервопривода и регулировкой скорости двигателей по расстоянию."
date: "2021-09-15"
...
graph TD
    ...
    F -->|Изменение коэффициента заполнения| G[Драйвер L298N H-Bridge]
    G -->|Управление DC-двигателями| H[Регулировка силы запуска]
```
...
2. **Регулировка силы запуска:** ультразвуковой датчик измеряет расстояние. Специальная функция преобразует его в коэффициент заполнения PWM-сигнала, управляющего двумя двигателями постоянного тока (DC) через мост L298N.

### Электрическая схема

На схеме показаны соединения Raspberry Pi, сервопривода, драйвера L298N и датчиков:

<img src="/images/ball_launcher_circuit.png" alt="Электрическая схема установки для запуска мячей" style="width: 80%; max-width: 600px; display: block; margin: 1.5rem auto; border-radius: 8px; border: 1px solid var(--border-color);" />
...
- **Коррекция ошибки:** координаты сравниваются с центром кадра. Если смещение превышает мёртвую зону $\pm 30$ пикселей, сервопривод получает PWM-сигнал и снова центрирует цель.

### 2. Кусочно-линейная регулировка дальности запуска

Порог запуска DC-двигателей и механическое сопротивление создают нелинейную связь между напряжением и дальностью. Для устойчивой работы на расстоянии от 50 до 210 см используется кусочно-линейная функция. Raspberry Pi получает расстояние от HC-SR04 и рассчитывает коэффициент заполнения PWM-сигнала:
...
<div class="video-container" style="position: relative; padding-bottom: 56.25%; height: 0; overflow: hidden; max-width: 100%; border-radius: 12px; margin: 2rem 0; box-shadow: var(--shadow-md); border: 1px solid var(--border-color);">
  <iframe
    ...
    title="Испытание установки Dodgeball для запуска мячей"
    frameborder="0"
