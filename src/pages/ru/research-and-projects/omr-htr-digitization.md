...
title: "Оцифровка OMR и HTR: система ExamOCR"
description: "Система оцифровки экзаменов с коррекцией перспективы, анализом меток и распознаванием последовательностей Qwen2.5-VL."
date: "2026-02-15"
...
**ExamOCR** выполняет весь процесс от сканированного листа до сохранённого результата. Система сочетает коррекцию перспективы, обрезку сеток по проекциям и визуально языковую модель Qwen2.5-VL-7B-Instruct в административном интерфейсе Streamlit.
...
graph TD
    ...
    D -->|Текстовые сетки вонгоджи| F[transcribe_qwen: движок VL OCR]
...
4. **Анализ клеток OMR:** проекции временных меток в `utils/detection.py`.
...
Первые прототипы искали каждое поле и клетку по отдельному контуру. В реальных сканах карандашные отметки, пересечения с рукописным текстом и складки листа разрушали границы и смещали выравнивание.
...
После нормализации листа области интереса извлекаются по фиксированным долям координат. Динамический поиск контуров больше не требуется:
```python
ROIS_P1 = {
...
### 3. OMR по проекциям временных меток

Для чтения заполненных клеток номера студента и даты рождения функция `read_omr_smart` вычисляет вертикальные и горизонтальные проекции бинарной плотности пикселей. Границы строк и столбцов находятся по пикам и впадинам распределения, которые работают как динамические **временные метки**.
...
Для постобработки разработан **лингвистический фильтр корейского языка**. Слог хангыля состоит из начальной согласной, средней гласной и необязательной конечной согласной. В диапазоне Unicode от `U+AC00` до `U+D7A3` возможно 11 172 комбинации, но стандарт **KS X 1001** содержит **2 350** распространённых корректных слогов.
